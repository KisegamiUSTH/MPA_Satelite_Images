...
% Run SSA enhancement once
disp('Running SSA optimization on GPU...');
...
output_file_name = 'ssa_enhanced_image.jpg';
imwrite(ssa_enhanced_img, output_file_name);
disp(['Enhanced image saved as: ', output_file_name]);

% Visualize the final enhanced image after SSA
figure;
subplot(1, 2, 1);
imshow(input_img, []);
title('Original Image (RGB)');
subplot(1, 2, 2);
imshow(ssa_enhanced_img, []);
title('SSA Enhanced');

% Step 3: Evaluate the enhancement using the valuation function
disp('Evaluating the enhancement...');
[psnr_value, ssim_value, mse_value, snr_value] = valuation(input_img, ssa_enhanced_img);
...
disp('SSA optimization process completed.');
