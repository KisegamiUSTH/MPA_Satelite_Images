function [enhanced_img, bestFitness] = woa_enhance(input_img)
    ...
    for generation = 1:numGenerations
        ...
        % Update positions using the WOA formula
        ...
        for i = 1:populationSize
            ...
            if p < 0.5
                if abs(A) < 1
                    ...
                    X_rand = positions(randIndex, :);
                    D = abs(C * X_rand - positions(i, :));
                    positions(i, :) = X_rand - A * D;
                end
            else
                D = abs(bestSolution - positions(i, :));
                positions(i, :) = D .* exp(-a * generation) .* cos(2 * pi * generation) + bestSolution;
            end
